...
func ListHosts(c *fiber.Ctx) error {
	...
	hosts, err := postgres.ListHosts(postgres.DB, tenantID, limit, offset)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{"error": "Cannot list hosts"})
	}
	return c.JSON(hosts)
}
...
func HostHeartbeat(c *fiber.Ctx) error {
	...
	err = postgres.UpdateHost(postgres.DB, id, map[string]interface{}{"agent_status": "online", "last_seen": time.Now()})
	if err != nil {
